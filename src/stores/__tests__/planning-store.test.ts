...
import { createTestStory, createTestSession, createTestPointCutoff } from '@/test/utils';
...
describe(usePlanningStore.name, () => {
  ...
  describe('updateStory', () => {
    ...
    it('should throw error for non-existent story', () => {
      ...
      expect(() => store.updateStory('non-existent', { title: 'New Title' }))
    ...
    it('should throw error when no active session', () => {
      ...
      expect(() => store.updateStory('story-1', { title: 'New Title' }))
  ...
  describe('updateStoryPosition', () => {
    beforeEach(() => {
      const story = createTestStory({ id: 'story-1', position: 0 });
      const testSession = createTestSession({ stories: [story] });
      usePlanningStore.setState({ currentSession: testSession });
    });
    ...
    it('should update story position', () => {
      ...
      store.updateStoryPosition('story-1', 50);
      
      const { currentSession } = usePlanningStore.getState();
      expect(currentSession?.stories[0].position).toBe(50);
    });

    it('should normalize position to valid range', () => {
      const store = usePlanningStore.getState();
      
      store.updateStoryPosition('story-1', 150); // Above max
      
      const { currentSession } = usePlanningStore.getState();
      expect(currentSession?.stories[0].position).toBe(100); // Normalized to max
    });

    it('should throw error for non-existent story', () => {
      const store = usePlanningStore.getState();
      
      expect(() => store.updateStoryPosition('non-existent', 50))
        .toThrow('Story not found');
    });
  });
  ...
  describe('deleteStory', () => {
    ...
    it('should promote new anchor when deleting anchor story', () => {
      const anchorStory = createTestStory({ id: 'anchor-1', isAnchor: true, position: 0 });
      const story1 = createTestStory({ id: 'story-1', isAnchor: false, position: 10 });
      const story2 = createTestStory({ id: 'story-2', isAnchor: false, position: -5 });
      const testSession = createTestSession({
      ...
      const { currentSession } = usePlanningStore.getState();
      expect(currentSession?.stories).toHaveLength(2);
      // story2 should become anchor (closest to position 0)
      const newAnchor = currentSession?.stories.find(s => s.isAnchor);
      expect(newAnchor?.id).toBe('story-2');
    ...
    it('should throw error for non-existent story', () => {
      ...
      expect(() => store.deleteStory('non-existent'))
        .toThrow('Story not found');
    });
  });
  ...
  describe('setAnchorStory', () => {
    ...
    it('should set new anchor story', () => {
      ...
      const { currentSession } = usePlanningStore.getState();
      expect(currentSession?.anchorStoryId).toBe('story-2');
      expect(currentSession?.stories.find(s => s.id === 'story-1')?.isAnchor).toBe(false);
      expect(currentSession?.stories.find(s => s.id === 'story-2')?.isAnchor).toBe(true);
    });

    it('should throw error for non-existent story', () => {
      const store = usePlanningStore.getState();
      
      expect(() => store.setAnchorStory('non-existent'))
        .toThrow('Story not found');
    });
  });
  ...
  describe('exportResults', () => {
    it('should export session results with story points', () => {
      const stories = [
        createTestStory({ id: 'story-1', title: 'Story 1', position: -60 }),
        createTestStory({ id: 'story-2', title: 'Story 2', position: -10 }),
        createTestStory({ id: 'story-3', title: 'Story 3', position: 30 }),
        createTestStory({ id: 'story-4', title: 'Story 4', position: 70 })
      ];
      
      const cutoffs: PointCutoff[] = [
        createTestPointCutoff({ position: -50, pointValue: 1 }),
        createTestPointCutoff({ position: 0, pointValue: 3 }),
        createTestPointCutoff({ position: 50, pointValue: 8 })
      ];
