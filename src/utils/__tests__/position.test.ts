...
describe(calculateRelativePosition.name, () => {
  const leftStory = createTestStory({ id: 'left', position: { x: -20, y: 0 } })
  const rightStory = createTestStory({ id: 'right', position: { x: 20, y: 0 } })
  ...
  it.each([
    ...
    'should calculate position with ratio %f as %i (%s)',
  ...
  it('should throw error when left position is not less than right position', () => {
    const invalidLeftStory = createTestStory({
      id: 'invalid',
      position: { x: 30, y: 0 },
    })
    expect(() =>
      calculateRelativePosition(invalidLeftStory, rightStory)
    ).toThrow('Left story must have a lower complexity than right story')
  })

  it('should normalize result to valid range', () => {
    const farLeftStory = createTestStory({
      id: 'farLeft',
      position: { x: -90, y: 0 },
    })
    const farRightStory = createTestStory({
      id: 'farRight',
      position: { x: 90, y: 0 },
    })
...
describe(findOptimalPosition.name, () => {
  it('should return anchor position for empty story list', () => {
    ...
    expect(result).toBe(ANCHOR_POSITION.x)
  })

  it('should position before first story when target is leftmost', () => {
    const stories = [createTestStory({ id: '1', position: { x: 10, y: 0 } })]
  ...
  it('should use target position when there is enough space between stories', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: -20, y: 0 } }),
      createTestStory({ id: '2', position: { x: 20, y: 0 } }),
    ]
  ...
  it('should calculate midpoint when gap is too small', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: -1, y: 0 } }),
      createTestStory({ id: '2', position: { x: 1, y: 0 } }),
    ]
    const result = findOptimalPosition(stories, 0)
  ...
  it('should handle multiple stories and find correct gap', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: -30, y: 0 } }),
      createTestStory({ id: '2', position: { x: -10, y: 0 } }),
      createTestStory({ id: '3', position: { x: 10, y: 0 } }),
      createTestStory({ id: '4', position: { x: 30, y: 0 } }),
    ]
    const result = findOptimalPosition(stories, 0)
...
describe(redistributePositions.name, () => {
  it('should return unchanged for single story', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: 0, y: 0 }, isAnchor: true }),
    ]
  ...
  it('should throw error when no anchor story exists', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: 0, y: 0 } }),
      createTestStory({ id: '2', position: { x: 10, y: 0 } }),
    ]
    expect(() => redistributePositions(stories)).toThrow(
      'No anchor story found for redistribution'
    )
  })

  it('should keep anchor story position unchanged', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: -50, y: 0 } }),
      createTestStory({ id: '2', position: { x: 0, y: 0 }, isAnchor: true }), // anchor
      createTestStory({ id: '3', position: { x: 50, y: 0 } }),
    ]
    const result = redistributePositions(stories)
    const anchorStory = result.find(s => s.isAnchor)
    expect(anchorStory?.position.x).toBe(0)
  })

  it('should redistribute stories evenly around anchor', () => {
    const stories = [
      createTestStory({ id: '1', position: { x: -90, y: 0 } }),
      createTestStory({ id: '2', position: { x: 0, y: 0 }, isAnchor: true }), // anchor
      createTestStory({ id: '3', position: { x: 90, y: 0 } }),
    ]
    const result = redistributePositions(stories)

    // Stories should be evenly spaced
    const sortedResult = result.sort((a, b) => a.position.x - b.position.x)
    expect(sortedResult[0].position.x).toBeLessThan(0) // Left of anchor
    expect(sortedResult[1].position.x).toBe(0) // Anchor
    expect(sortedResult[2].position.x).toBeGreaterThan(0) // Right of anchor
  })
...
describe(findClosestStory.name, () => {
  ...
  beforeEach(() => {
    stories = [
      createTestStory({ id: '1', position: { x: -20, y: 0 } }),
      createTestStory({ id: '2', position: { x: 0, y: 0 } }),
      createTestStory({ id: '3', position: { x: 30, y: 0 } }),
    ]
  })
...
describe(snapToGrid.name, () => {
  ...
  it.each([
    ...
    'should snap %i to %i with grid size %i (%s)',
    (input, gridSize, expected) => {
