...
// Test data helpers
const createTestStory = (overrides: Partial<Story> = {}) => fromPartial<Story>({
...
const createTestSession = (overrides: Partial<PlanningSession> = {}) => fromPartial<PlanningSession>({
    id: 'session-1',
    name: 'Test Session',
    stories: [],
    anchorStoryId: null,
...
const createMockStore = (session: PlanningSession | null) => fromPartial<PlanningStore>({
...
describe('PlanningCanvas Integration Tests', () => {
    ...
    describe('Drag and Drop Integration', () => {
        it('integrates with DndProvider for story positioning', () => {
            const stories = [
                createTestStory({
                    id: 'story-1',
            ...
            expect(screen.getByLabelText('Drag to reposition story')).toBeInTheDocument();
        });
    ...
    describe('Canvas Responsiveness', () => {
        it('adapts to different screen sizes', () => {
            ...
            const positionSummary = screen.getAllByText('Story Positions')[0].nextElementSibling;
            expect(positionSummary).toHaveClass('grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3');
        });
    ...
    describe('User Interaction Integration', () => {
        it('integrates click handlers with story cards', () => {
            ...
            expect(handleStoryClick).toHaveBeenCalledWith(
        ...
        it('maintains keyboard accessibility during interactions', () => {
            ...
            const canvases = screen.getAllByRole('application');
            const canvas = canvases.find(c => c.getAttribute('aria-label') === 'Story positioning canvas');
            const storyCard = screen.getByRole('button', { name: /Story: Keyboard Story/ });
