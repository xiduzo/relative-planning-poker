...
const createTestSession = (overrides: Partial<PlanningSession> = {}): PlanningSession => ({
  id: 'session-1',
  name: 'Test Session',
  stories: [],
  anchorStoryId: null,
...
describe(PlanningCanvas.name, () => {
  ...
  describe('Empty State', () => {
    ...
    it('displays empty state when session has no stories', () => {
      ...
      expect(screen.getByText('Add your first story to start planning')).toBeInTheDocument();
    });
    ...
    it('renders droppable area with proper accessibility attributes', () => {
      ...
      const canvas = emptyMessages[0].closest('div[class*="border-dashed"]');
      expect(canvas).toHaveClass('border-dashed');
    });
  });
  ...
  describe('Horizontal Axis Visualization', () => {
    ...
    it('renders canvas with proper accessibility attributes', () => {
      ...
      const canvas = canvases.find(c => c.getAttribute('aria-label') === 'Story positioning canvas');
  ...
  describe('Story Positioning', () => {
    ...
    it('applies correct z-index for anchor stories', () => {
      ...
      const anchorStoryCard = screen.getByRole('button', { name: /Story: Anchor Story/ });
      const anchorStoryContainer = anchorStoryCard.closest('div[style*="z-index"]');
  ...
  describe('Position Summary', () => {
    ...
    it('sorts stories by position in summary', () => {
      ...
      expect(screen.getAllByText('Center Story').length).toBeGreaterThanOrEqual(2);
      expect(screen.getAllByText('Right Story').length).toBeGreaterThanOrEqual(2);
    });
    ...
    it('highlights anchor story in position summary', () => {
      ...
      expect(screen.getAllByText('Regular Story').length).toBeGreaterThanOrEqual(2);
      expect(screen.getAllByText('Anchor Story').length).toBeGreaterThanOrEqual(2);
  ...
  describe('Responsive Design', () => {
    it('applies responsive grid classes to position summary', () => {
      ...
      expect(summaryGrid).toHaveClass('grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3');
    });
    ...
    it('constrains story positioning within canvas bounds', () => {
      ...
      const storyElements = screen.getAllByRole('button').filter(el => 
        el.getAttribute('aria-label')?.startsWith('Story:')
      );
  ...
  describe('Accessibility', () => {
    ...
    it('maintains focus management for keyboard navigation', () => {
      ...
      const canvases = screen.getAllByRole('application');
      const canvas = canvases.find(c => c.getAttribute('aria-label') === 'Story positioning canvas');
      expect(canvas).toHaveClass('focus-within:outline-none', 'focus-within:ring-2');
    });
  });
});