...
export const StorySchema = z.object({
  ...
  title: z.string()
    ...
    .max(STORY_TITLE_MAX_LENGTH, `Story title must be ${STORY_TITLE_MAX_LENGTH} characters or less`),
  description: z.string()
    .max(STORY_DESCRIPTION_MAX_LENGTH, `Story description must be ${STORY_DESCRIPTION_MAX_LENGTH} characters or less`),
  position: z.number()
    .min(POSITION_MIN, `Story position must be between ${POSITION_MIN} and ${POSITION_MAX}`)
    .max(POSITION_MAX, `Story position must be between ${POSITION_MIN} and ${POSITION_MAX}`),
...
export const PointCutoffSchema = z.object({
  ...
  position: z.number()
    .min(POSITION_MIN, `Position must be between ${POSITION_MIN} and ${POSITION_MAX}`)
    .max(POSITION_MAX, `Position must be between ${POSITION_MIN} and ${POSITION_MAX}`),
  pointValue: z.number()
    .refine(val => FIBONACCI_NUMBERS.includes(val as any), {
